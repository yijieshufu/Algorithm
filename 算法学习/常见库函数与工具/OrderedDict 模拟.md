...
# 分析
- **数据结构选择**：
    - 题目要求频繁地进行“查找”、“删除”和“移动到首部”操作。
    - 在 Python 中，`collections.OrderedDict` 是实现此类逻辑的“神兵利器”。它结合了哈希表（查找 $O(1)$）和双向链表（移动/删除 $O(1)$）的特性。
- **规则优先级**：
    - 访问一个已在 $q_2$ 中的数据，不仅要从 $q_2$ 删除，还要将其加入 $q_1$。这可能触发 $q_1$ 的淘汰，进而可能触发向 $q_2$ 的移动。
- **细节坑点**：
    - **Rule 4** 明确指出：从 $q_1$ 淘汰出的数据只有在 **$q_2$ 未满** 时才移入 $q_2$。这意味着如果 $q_2$ 已经满了，$q_1$ 溢出的数据就直接消失了。
## 代码
